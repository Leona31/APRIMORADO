...
client.on('group-participants-update', async (anu) => {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: '❬❗❭AGUARDE UM POUCO, ESTOU PROCESSANDO ❬❗❭',
				success: '️❬ ✔ ❭ PRONTINHO 🖤',
				error: {
				...
				only: {
					...
					admin: ' ❬ ⚠️ ❭ IIII ALA MEMBRO COMUM TENTANDO USAR COMANDOS DE ADMS KKKKK ❬ ⚠️ ❭ ',
...
case 'lofi':
                 ...
                 case 'logoph':
					...
					var gbl1 = gh.split("|")[0];
					var gbl2 = gh.split("|")[1];
					if (args.length < 1) return reply('escolha um txt pfvr')
					reply(mess.wait)
			...
			case 'mapa': 
				...
				case 'figurinha':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply(`sapoha ${prefix}sticker e pra fazer fig cm foto/gif/video de 10seg só tbm`)
					}
					break
				case 'gtts':	
				case 'tts':
				...
				await client.client.leaveGroup(from, 'adeus...', groupId)
                    break
				case 'bc': 
					if (!isOwner) return reply(' SÓ O MARIANO PODE ALTERAR ISSO') 
			...
			case 'remover':
				...
				case 'sticker':
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						reply(`Envie fotos com legendas ${prefix}sticker ou a tag da imagem enviada mané`)
					}
					break
				case 'welcome':
				case 'bv':
